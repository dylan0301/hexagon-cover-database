...
For a closed center triangle define
\[
 N_E(T_C)=
...
Every center triangle has exactly one of the types CE0, CE1, and CE2.
Equivalently,
\[
 N_E(T_C)\le2.
\]
...
For an original vertex triangle $T_i$, let
\[
...
Every original vertex triangle has $1\le o\le3$.  If it has positive-length
...
Every original vertex triangle has exactly one of the four types Vd0, Vd1, Vd2,
and T3-like.
\end{proposition}
...
\subsection{Reach coordinates, supercritical vertex triangles, and gaps}

For the original vertex triangle $U_i$, define its maximal incoming,
...
On each edge the complement of the two incident vertex-triangle traces is either
...
nonincident vertex triangle cannot contain a relative-interior point of the edge:
...
\begin{lemma}[T3-like vertex triangles are nonsupercritical]
\label{lem:t3-nonsupercritical}
Every original T3-like vertex triangle satisfies
\begin{equation}
 A_i+B_i\le1.
...
Its hypothesis that the six vertex triangles themselves cover the boundary is
...
Moreover:
\begin{enumerate}
 \item if exactly one actual vertex triangle, indexed by $p$, is supercritical, every
 strict selection has exactly one selected supercritical vertex triangle, also indexed
 by $p$;
 \item if at least two actual vertex triangles are supercritical, some simultaneous
 strict selection has at least two selected supercritical vertex triangles.
\end{enumerate}
\end{proposition}
...
\]
No nonincident vertex triangle contains a relative-interior point of
...
For the other four nonsupercritical vertex triangles, successive inequalities
...
\eqref{eq:actual-selected-ascents}, both adjacent vertex triangles are selected
supercritical.
\end{proof}

\subsection{Midpoint forcing}

We first need a local fact for vertex triangles.  Take unit vectors
\[
...
\]
Then $a+b\le1$.  Consequently, for every vertex triangle,
\[
...
distinct center and vertex triangles used in Table~\ref{tab:routing}, and its
maximal reaches determine exactly one vertex triangle of that table.
\end{proposition}
...
classification give the five displayed vertex triangles.
...
either enters the first CE1/CE2 vertex triangle or has $q\le2$ and $N_+\le1$.  If
