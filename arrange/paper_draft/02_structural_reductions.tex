...
Moreover, under a hypothetical cover,
\[
 N_{\mathrm{gap}}=0
...
stated bounds, including singleton gaps.  Since every boundary gap lies in a
positive center trace, $N_{\mathrm{gap}}=0$ is equivalent to the absence of
all boundary gaps, which is exactly coverage of $\partial H$ by the six open
V triangle traces.
\end{proof}
...
center and vertex types.  The maximal reaches determine $N_+$, while the
actual open boundary traces determine $N_{\mathrm{gap}}$.

First suppose $N_{\mathrm{gap}}=0$.  Lemma~\ref{lem:gap-exhaustion} makes
the six V triangles boundary-complete, independently of the center class.
If $N_+=0$ or $N_+\ge2$, the first or fifth zero-gap row applies.  If
$N_+=1$, the vertex classification gives exactly one of $d\ge1$,
$d=0,t\ge1$, and $(d,t)=(0,0)$.  These are the remaining three zero-gap
rows.

Now suppose $N_{\mathrm{gap}}\ge1$.  Lemma~\ref{lem:gap-exhaustion} forces
the center to be CE1 or CE2, and Proposition~\ref{prop:unique-center-midpoint}
gives its unique radial midpoint.  If $N_+\ge2$, Lemma
\ref{lem:positive-support-rescuer} supplies a distinct positive-support V
triangle, so $N_{\mathrm{sp}}\ge1$ and
Theorem~\ref{thm:common-skeleton-count} applies.  For $N_+\in\{0,1\}$, the
same theorem removes $N_++d+t\ge3$.  Every survivor therefore has $N_+=0$
with $d+t\le2$, or $N_+=1$ with $d+t\le1$.  In the former case the exhaustive
alternatives are $(d,t)=(0,0)$, $d\ge1$, and $d=0$ with
$t\in\{1,2\}$.  In the latter case they are $(0,0)$, $(0,1)$, and $(1,0)$.
The final alternative separates by center type: CE1 is the length row and
CE2 is the hybrid placement row.  These gap-first alternatives are disjoint
and exhaustive.
\end{proof}
