...
For zero gaps, boundary completeness holds independently of center type;
the three possibilities $N_+=0,1,\ge2$ are disjoint. For a nonzero gap,
the C triangle is CE1 or CE2. First separate $N_+=0$, then the high-count
states $N_+\ge2$ or $N_++N_{\rm sp}\ge3$. Every survivor has
$N_+=1,N_{\rm sp}\le1$. Thus the rows of \zcref{tab:routing} are
disjoint and exhaustive. The later midpoint-supplier lemma resolves the
last row by relative placement, rather than by a list of V-type patterns.
\end{proof}
