...
\[
 A(T_{\mathrm{next}})\ge\widehat g_c^\vee(a)\ge a.
\]
\end{lemma}

\begin{proof}
The closure of $T$ realizes $(a,B(T),c)$, so the definition of $g_c$ gives
the outgoing bound. On a center-free edge, coverage gives
$A(T_{\mathrm{next}})\ge1-B(T)$, and complementation proves the raw transfer.
Under nonsupercriticality, $B(T)\le1-A(T)\le1-a$, so
$B(T)\le\min\{g_c(1-a),1-a\}=\widehat g_c(1-a)$. The final assertion follows
in the same way.
\end{proof}
...
\]
Apply these maps with $p=B(T)$ and $q=A(T_{\mathrm{next}})$, where $T$ is
an actual V triangle satisfying $A(T)\ge a$ and $C(T)\ge c$, and
$T_{\mathrm{next}}$ is the next incident actual V triangle.
Then
\begin{equation}
 A(T_{\mathrm{next}})\ge g_{c,J}^\vee(a),
 \label{eq:reader-generalized-raw-transfer}
\end{equation}
and the hatted version holds when $T$ is nonsupercritical:
\begin{equation}
 A(T_{\mathrm{next}})\ge\widehat g_{c,J}^\vee(a).
