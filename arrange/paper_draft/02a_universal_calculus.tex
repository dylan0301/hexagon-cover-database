...
Let $T$ be an actual vertex triangle satisfying
$A(T)\ge a$ and $C(T)\ge c$, and let $T_{\mathrm{next}}$ be the next
incident actual vertex triangle. Then
\[
 B(T)\le g_c(1-a).
\]
...
vertex triangle satisfying $A(T)\ge a$ and $C(T)\ge c$.  Then
\begin{equation}
...
\end{equation}
The supremum is not attained. Hence an actual strict-supercritical vertex triangle
...
the next incident actual vertex triangle, then coverage gives
\begin{equation}
...
\end{equation}
Consequently, if all $k$ internal vertex triangles are nonsupercritical, the chain is
impossible whenever $u+v<1$.
\end{lemma}

\begin{proof}
The first and last component hypotheses give $A_1\ge1-u$ and
$B_k\ge1-v$.  On every middle edge the center and all nonincident triangles have
...
\[
 ...
 \min\{e(d_1),e(d_2)\}<Q,
\]
then at least one of those two vertex triangles forces every subsequent output above
$1-Q$.
\end{lemma}
