...
skeleton estimate below is conditional on its stated center and vertex triangle
...
If $T_C$ is a CE1 or CE2 center triangle and contains exactly one radial
midpoint, then
\[
 L_S(T_C)<\frac32.
\]
...
Let $T_i$ be the closure of an original open vertex triangle, so
...
triangle for $\widetilde H$.  Its positive adjacent-arm trace is a positive
...
If a vertex triangle has $A_i+B_i\le1$ and no positive-length trace on either
...
\begin{proof}
By Proposition~\ref{prop:vertex-classification}, a vertex triangle is Vd0,
Vd1, Vd2, or T3-like.  Proposition~\ref{prop:vd-corner-normal-form} makes
every Vd1/Vd2 vertex triangle nonsupercritical, and
Lemma~\ref{lem:t3-nonsupercritical} does the same for T3-like vertex triangles.  Hence a
supercritical triangle is Vd0 and, by definition, has no positive-length trace on
...
at least two vertex triangles are supercritical, then a vertex triangle distinct from
a chosen supercritical triangle has positive-length adjacent-arm support.
\end{lemma}
...
vertex triangle.  Diameter one restricts this triangle to $U_{s-1}$ or $U_{s+1}$.
...
If an original Vd2 triangle contains either neighboring midpoint, then its exact
...
The CE0 vertex triangle is the definition.  Both remaining center vertex triangles are the two class
specializations of the common signed formula in
Corollary~\ref{cor:signed-center-boundary}.

For a nonsupercritical Vd0 vertex triangle, \eqref{eq:vertex-boundary-locality} is enough.
For a supercritical vertex triangle, its incident endpoints are at squared distance
$A_i^2+A_iB_i+B_i^2\le1$, and hence
\[
 \frac34(A_i+B_i)^2\le1.
\]
This proves the $2/\sqrt3$ cap.  The Vd1/Vd2 vertex triangle is
...
\begin{enumerate}
 \item CE0 with $N_+=0$;
 \item CE0 with $N_+=1$ and at least one Vd1/Vd2 triangle;
 \item CE1 or CE2 with $N_+=0$ and at least one Vd1/Vd2 triangle;
 \item CE1 with $N_+=1$ and at least one Vd1/Vd2 triangle;
 \item CE2 with $N_+=1$ and at least two Vd1/Vd2 triangles.
\end{enumerate}
\end{proposition}

\begin{proof}
In CE0, the center open triangle contains no relative-interior boundary point.
No nonincident vertex triangle can contain such a point either.  Hence on every
...
is Vd2 and contains a neighboring midpoint, and all other vertex triangles are
...
\begin{enumerate}
 ...
 \item the Vd2 neighboring-midpoint subcase of the CE2 hybrid routing entry.
\end{enumerate}
\end{proposition}

\begin{proof}
Items (1), (2), (4), and (5) are
Proposition~\ref{prop:boundary-length-branches}.  Item (3) is the short-triangle
count and the three-short-triangle theorem in
...
subcase of the exceptional CE2 vertex triangle; its remaining placements are handled by
