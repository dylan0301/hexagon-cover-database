\section{Exact formulas and technical proofs for Strategy 2}
\label{app:strategy2-details}

This appendix treats the configurations in which a coarse length sum does not
...
\[
 ...
 w=\frac{y}{x+y}.
\]
The far endpoint roles have inputs at least $p$ and $q$, with radial demands
$p/w$ and $q/r$.  By Lemma~\ref{lem:two-endpoint-capped-loss}, their two
...
\end{equation}
Apply Lemma~\ref{lem:half-edge-radial-envelope} with
$p=1/2+A$ and $h=H$.  Equation~\eqref{eq:adjacent-AH} gives
\[
...
\end{equation}
The exact CE2 coupling, the disk constraints for $T_0$ and the center role,
and \eqref{eq:adjacent-AH} are precisely the hypotheses of
Lemma~\ref{lem:ce2-adjacent-outer-ratio}.  Therefore
\begin{equation}
 d_2^C<\frac H3.
