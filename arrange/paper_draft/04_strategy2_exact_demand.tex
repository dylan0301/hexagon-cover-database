...
\]
Let $A$ and $D$ denote the center exits called $C_0$ and $C_2$,
respectively, in Proposition~\ref{prop:center-exact-formulas}; they are not
the actual vertex-role radial reaches denoted by $C_i$.
...
\]
The gap gives $B_0\ge s$ and $A_1\ge X$.  Define
\[
 z_0=X,\qquad z_j=G_{c_j}(z_{j-1})\quad(1\le j\le5).
\]
We now justify the fivefold propagation.  Suppose $A_j\ge z_{j-1}$ for
some $1\le j\le5$.  Radial coverage gives actual radial reach at least
$c_j$, and row $j$ is nonsupercritical.  Hence the safe capped bound
\eqref{eq:safe-capped-bound} and the monotonicity of $F_{c_j}$ give
\[
 B_j\le F_{c_j}(z_{j-1}).
\]
There is no V-gap on the next edge: for $j=1,\ldots,4$ that edge has no
center trace, and for $j=5$ the CE1 center has no trace on $e_{5,0}$.
The strict open-trace handoff has the weak consequence
\[
 A_{j+1}\ge1-B_j\ge1-F_{c_j}(z_{j-1})=z_j,
\]
where $A_6$ means $A_0$.  Starting from $A_1\ge z_0$ proves
\begin{equation}
 A_0\ge z_5=:Z
 =(G_{c_5}\circ G_{c_4}\circ G_{c_3}\circ G_{c_2}\circ G_{c_1})(X).
 \label{eq:ce1-fivefold-propagation}
\end{equation}
On row $0$, the inequalities $B_0\ge s$ and actual radial reach at least
$c_0$, followed by reflection and down-closedness of $\mathcal A$, show that
$(s,A_0,c_0)\in\mathcal A$.  Therefore
\begin{equation}
 A_0\le B_{c_0}(s).
 \label{eq:ce1-supercritical-target}
\end{equation}

It remains to prove $Z>B_{c_0}(s)$.  Since $G_{c_1}$ and $G_{c_5}$ are
extensive and the middle maps are nondecreasing, it is enough first to prove
\[
 (G_{c_4}\circ G_{c_3}\circ G_{c_2})(X)>1-H.
\]
The negation of this inequality is, by three explicit applications of
\eqref{eq:S2-capped-duality}, equivalent to
\begin{align*}
...
\]
Thus $H>A$.  Since $H=s/2$ and $0<s<t\le1$, we also have
$H<1/2<1-A$.
...
and the two center inequalities give
\[
 Q(A):=\eta+P+wA-2R(2A+5A^2)\le0,
 \qquad 0<A<A_\star:=P-w(R-\tfrac12).
\]
...
\]
When $R\le1/2$, one has $k\le2-\sqrt3<27/100$ and $A_\star\ge P$, while
\[
...
When $R>1/2$, $0<A_\star<P$ and
\[
 Q(A_\star)=\frac{(1-2k)(-g_5(k))}{2(1-k^2)^3},\qquad
...
$Q(A_\star)>0$.  In either range concavity makes $Q(A)>0$ throughout the
...
$2R(H-m)=\eta+D-A>\eta-P>0$.  Extensivity gives
$p_1\ge H>m$, while $p_1<1/2<1-m$; thus both Full ranges are impossible.
On $L$, $T_-$, or the low-$c$ tie one has $F_{1-m}(p_1)\le p_1$, so
\[
 G_{1-m}(p_1)\ge1-p_1>1-X.
\]
...
$T_+$ branch.  We use the following elementary property of a
...
\[
 \mathsf A=f'(q)=1+d-2q,\qquad
 \mathsf B=g'(u)=2(1-d)^2(1-u),
\]
then
\[
 u'=\frac{\mathsf A}{\mathsf B},\qquad
 u''=\frac{2((1-d)^2\mathsf A^2-\mathsf B^2)}{\mathsf B^3}.
\]
Moreover
\[
 \mathsf B-(1-d)\mathsf A=(1-d)(1-3d+2p+2du)>0
\]
and
\[
 \mathsf B-\mathsf A\ge(1-d)(1-2d)>0.
\]
...
row-$3$ parameter $d=m$, which need not be at most $\mu$, the remaining
...
$e(D)<2D+3D^2<p_2$.  To check every row-$2$ label explicitly, put
\[
 e_D=e(D)=\ell(1-D),\qquad r_D=1-D-e_D.
\]
Since $D<1/10$, one has $c_2=1-D>\sqrt3/2$, and the exact catalog is
\[
 \begin{array}{c|c|c}
 ...
 \end{array}
\]
The inequality $p_2>e_D$ excludes the first two ranges.  On the $L$ range
$F_{1-D}(p_2)=e_D$; on the $T_-$ range, monotonicity and
$F_{1-D}(r_D)=e_D$ give $F_{1-D}(p_2)\le e_D$.  Hence in both middle
ranges
\[
 G_{1-D}(p_2)\ge1-e_D>1-X,
\]
because
\[
...
\]
because $X>1/2>D$.  Thus every row-$2$ label gives the strict inequality
\eqref{eq:ce1-suffix}.
...
\[
 Z>1-H=1-\frac s2.
\]
For any admissible pair of boundary reaches $s,b$, the diameter constraint is
$s^2+sb+b^2\le1$.  Its positive endpoint
\[
 \beta(s)=\frac{-s+\sqrt{4-3s^2}}2
\]
therefore satisfies $B_{c_0}(s)\le\beta(s)$.  Since $s>0$,
$\sqrt{4-3s^2}<2$, and so $\beta(s)<1-s/2$.  Thus
$Z>B_{c_0}(s)$, contradicting
\eqref{eq:ce1-fivefold-propagation} and
\eqref{eq:ce1-supercritical-target}.
\end{proof}
...
Since $E\ge\sqrt3/2$ and $E(1-E)$ decreases on this range,
\begin{equation}
 0<\alpha,\delta<P\le\mu<1-\frac{\sqrt3}{2}.
 \label{eq:ce2-root-range}
\end{equation}
Thus all low-root estimates used below are within their stated ranges.

Suppose first that the gap is in $[y,v]$; then $[x,u]$ contains no V-gap.
The endpoint bounds give
\[
 B_0\ge y,\qquad A_1\ge z_0:=1-v=R-\delta.
\]
Define all five successive map inputs before using them:
\begin{equation}
 z_i=G_i(z_{i-1})\quad(1\le i\le5),
 \qquad
 Z_R=z_5=(G_5\circ G_4\circ G_3\circ G_2\circ G_1)(1-v).
 \label{eq:ce2-right-chain}
\end{equation}
As in the CE1 proof, these scalar inputs are dominated by the actual row
reaches.  Indeed, if $A_i\ge z_{i-1}$, then radial coverage, the
nonsupercritical cap, and monotonicity give
\[
 B_i\le F_{c_i}(z_{i-1}).
\]
For $i=1,\ldots,4$ the next edge has no center trace, while for $i=5$ the
other center interval $[x,u]$ contains no V-gap.  Hence the next open-trace
handoff gives, with $A_6=A_0$,
\[
 A_{i+1}\ge1-B_i\ge G_i(z_{i-1})=z_i.
\]
Induction from $A_1\ge z_0$ proves
\begin{equation}
 A_0\ge Z_R.
 \label{eq:ce2-right-actual}
\end{equation}
On the supercritical row, $B_0\ge y$ and the actual radial reach is at least
$c_0$.  Reflection and down-closedness of $\mathcal A$ therefore give
\begin{equation}
 A_0\le B_{c_0}(y).
 \label{eq:ce2-right-target}
\end{equation}

We next estimate the formal chain.  The second inequality in
\eqref{eq:ce2-normal-domain} and the identity $Rw=\eta+P$ give
\[
...
\]
Because $0<\alpha<P$, concavity gives $\pi(\alpha)>0$.  Together with
\eqref{eq:low-root-bounds} and \eqref{eq:ce2-root-range}, this proves
\begin{equation}
...
\end{equation}
Suppose instead that both roots are at least $Q$.  The strict rational
low-root bounds then imply
\[
 \alpha>L,\qquad\delta>L,
 \qquad L=\frac{Q}{2(1+Q)}.
\]
Since $k=2RQ=\eta+\alpha+\delta$, we obtain
\[
 \eta<Q\left(2R-\frac1{1+Q}\right).
\]
On the other hand, the first inequality in
\eqref{eq:ce2-normal-domain} gives
\begin{align*}
 \eta E&>\delta+R\alpha
...
\]
Combining the strict upper and lower bounds for $\eta$ gives $H(Q)>0$, where
\[
...
Thus $H(Q)<H(w/(2R))<0$, a contradiction.  This proves
\eqref{eq:ce2-two-threshold}.

Every $G_i$ is extensive.  If $e(\alpha)<Q$, then
$z_3\ge z_0>e(\alpha)$, and $c_4=1-\alpha$ together with
\eqref{eq:high-demand-threshold} gives
\[
 z_4\ge1-e(\alpha)>1-Q.
\]
Thus $z_5>1-Q$.  Otherwise $e(\alpha)\ge Q$, so
\eqref{eq:ce2-two-threshold} and \eqref{eq:ce2-initial-threshold} give
\[
...
\[
 B_{c_0}(y)\le\beta(y)<1-\frac y2.
\]
This contradicts \eqref{eq:ce2-right-actual} and
\eqref{eq:ce2-right-target}.

It remains to record the other orientation explicitly.  Reflection across
the axis through $V_0$ exchanges
\[
 (x,u,R,\alpha)\longleftrightarrow(y,v,w,\delta)
\]
and transforms the demand sequence by
\[
 (c_1,c_2,c_3,c_4,c_5)
 \longleftrightarrow(c_5,c_4,c_3,c_2,c_1).
\]
Suppose now that the gap is in $[x,u]$, so $[y,v]$ contains no V-gap.  Its
endpoint bounds give
\[
 A_0\ge x,\qquad B_5\ge\widetilde z_0:=1-u.
\]
Define the reflected chain by
\[
 \widetilde z_1=G_5(\widetilde z_0),\quad
 \widetilde z_2=G_4(\widetilde z_1),\quad
 \widetilde z_3=G_3(\widetilde z_2),\quad
 \widetilde z_4=G_2(\widetilde z_3),\quad
 \widetilde z_5=G_1(\widetilde z_4).
\]
The same one-row argument, now starting with the actual reach $B_5$ and
traversing rows $5,4,3,2,1$, gives successively
\[
 A_j\le F_{c_j}(\widetilde z_{5-j}),\qquad
 B_{j-1}\ge1-A_j\ge\widetilde z_{6-j}
 \quad(j=5,4,3,2,1).
\]
The last handoff is valid because the other center interval $[y,v]$ has no
V-gap.  In particular,
\begin{equation}
 B_0\ge Z_L:=\widetilde z_5
 =(G_1\circ G_2\circ G_3\circ G_4\circ G_5)(1-u).
 \label{eq:ce2-left-actual}
\end{equation}
Meanwhile the actual row-$0$ reaches satisfy $A_0\ge x$ and $C_0\ge c_0$.
Coordinatewise down-closedness gives $(x,B_0,c_0)\in\mathcal A$, and hence
\begin{equation}
 B_0\le B_{c_0}(x).
 \label{eq:ce2-left-target}
\end{equation}
If $G_i'$ denotes the $i$th map after reflection, then
\[
 G_1'=G_5,\quad G_2'=G_4,\quad G_3'=G_3,
 \quad G_4'=G_2,\quad G_5'=G_1.
\]
The normalized domain, initial-threshold proof, and two-threshold proof are
invariant under the displayed exchange.  Applying the right-chain argument
to the reflected variables therefore gives
\[
 Z_L>1-\frac x2>B_{c_0}(x),
\]
where the last inequality is the same diameter estimate with $x$ in place of
$y$.  This contradicts \eqref{eq:ce2-left-actual} and
\eqref{eq:ce2-left-target}, completing both orientations.
\end{proof}
