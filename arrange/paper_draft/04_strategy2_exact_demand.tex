...
\]
Radial coverage gives row $j$ actual radial reach at least $c_j$, and rows
$1,\ldots,5$ are nonsupercritical.  Hence
\eqref{eq:safe-capped-bound} and the monotonicity of $F_{c_j}$ apply at
each step.  There is no V-gap on the next edge: for $j=1,\ldots,4$ that
edge has no center trace, and for $j=5$ the CE1 center has no trace on
$e_{5,0}$.  The five actual-row transitions are therefore
\begin{align*}
 A_1&\ge z_0,&
 B_1&\le F_{c_1}(z_0),&
 A_2&\ge1-B_1\ge G_{c_1}(z_0)=z_1,\\
 A_2&\ge z_1,&
 B_2&\le F_{c_2}(z_1),&
 A_3&\ge1-B_2\ge G_{c_2}(z_1)=z_2,\\
 A_3&\ge z_2,&
 B_3&\le F_{c_3}(z_2),&
 A_4&\ge1-B_3\ge G_{c_3}(z_2)=z_3,\\
 A_4&\ge z_3,&
 B_4&\le F_{c_4}(z_3),&
 A_5&\ge1-B_4\ge G_{c_4}(z_3)=z_4,\\
 A_5&\ge z_4,&
 B_5&\le F_{c_5}(z_4),&
...
\]
one has $-f_8(k)=p(k)>0$ throughout the required interval, and so
$Q(P)>0$.
...
\[
 -g_5'(k)
 =\frac{29-5y^4+y\left(39(y-1)^2+12\right)}8>0.
\]
Thus $g_5$ decreases and remains negative for $k>k_0>1/4$, so
...
\[
 ...
 \end{aligned}
\]
Write $F_i=F_{c_i}$ and $G_i=G_{c_i}$ for $1\le i\le5$.
...
\end{equation}
Radial coverage, the nonsupercritical cap, and monotonicity bound each
actual outgoing reach.  For rows $1,\ldots,4$ the next edge has no center
trace, while after row $5$ the other center interval $[x,u]$ contains no
V-gap.  All five handoffs are therefore strict.  The complete actual-row
chain is
\begin{align*}
 A_1&\ge z_0,&
 B_1&\le F_{c_1}(z_0),&
 A_2&>1-B_1\ge G_1(z_0)=z_1,\\
 A_2&>z_1,&
 B_2&\le F_{c_2}(z_1),&
 A_3&>1-B_2\ge G_2(z_1)=z_2,\\
 A_3&>z_2,&
 B_3&\le F_{c_3}(z_2),&
 A_4&>1-B_3\ge G_3(z_2)=z_3,\\
 A_4&>z_3,&
 B_4&\le F_{c_4}(z_3),&
 A_5&>1-B_4\ge G_4(z_3)=z_4,\\
 A_5&>z_4,&
 B_5&\le F_{c_5}(z_4),&
 A_0&>1-B_5\ge G_5(z_4)=z_5.
\end{align*}
In particular,
\begin{equation}
 A_0>Z_R.
...
It remains to record the other orientation explicitly.  Write primes for
the data after reflection across the axis through $V_0$.  The normalized
center variables exchange as
\[
 (x,u,R,\alpha)\longleftrightarrow(y,v,w,\delta)
\]
and the actual boundary reaches transform as
\[
 A'_0=B_0,\qquad B'_0=A_0,\qquad
 A'_i=B_{6-i},\qquad B'_i=A_{6-i}\quad(1\le i\le5).
\]
The radial demands and capped maps transform in the exact order
\[
 c'_0=c_0,\qquad c'_i=c_{6-i},\qquad
 F'_i=F_{6-i},\qquad G'_i=G_{6-i}\quad(1\le i\le5).
\]
...
\]
Starting from the actual reach $B_5$ and traversing rows $5,4,3,2,1$, the
five reverse transitions are
\begin{align*}
 B_5&\ge\widetilde z_0,&
 A_5&\le F_{c_5}(\widetilde z_0),&
 B_4&>1-A_5\ge G_5(\widetilde z_0)=\widetilde z_1,\\
 B_4&>\widetilde z_1,&
 A_4&\le F_{c_4}(\widetilde z_1),&
 B_3&>1-A_4\ge G_4(\widetilde z_1)=\widetilde z_2,\\
 B_3&>\widetilde z_2,&
 A_3&\le F_{c_3}(\widetilde z_2),&
 B_2&>1-A_3\ge G_3(\widetilde z_2)=\widetilde z_3,\\
 B_2&>\widetilde z_3,&
 A_2&\le F_{c_2}(\widetilde z_3),&
 B_1&>1-A_2\ge G_2(\widetilde z_3)=\widetilde z_4,\\
 B_1&>\widetilde z_4,&
 A_1&\le F_{c_1}(\widetilde z_4),&
 B_0&>1-A_1\ge G_1(\widetilde z_4)=\widetilde z_5.
\end{align*}
The last strict handoff is valid because the other center interval $[y,v]$
has no V-gap.  In particular,
\begin{equation}
 B_0>Z_L:=\widetilde z_5
