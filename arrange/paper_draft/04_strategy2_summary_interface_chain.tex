...
\item A branch-specific affine, threshold, quarter-radial, or Vd-corner lower
bound whose domain and pointwise validity are stated in
...
\caption{The geometric content of a relaxed transfer chain.  Endpoint center
traces are first absorbed into residual demands.  Only the verified
center-free internal handoffs may then be weakened to identity propagation.}
...
\begin{proof}
The handoff induction is Proposition~\ref{prop:body-transfer-interface}.
The endpoint conclusion is Lemma~\ref{lem:reader-boundary-path}.  The strict
upper bound in the returned-demand case is the exact one-gap actual-V-triangle
interface.  The radial cases use the branch-specific terminal statements in
