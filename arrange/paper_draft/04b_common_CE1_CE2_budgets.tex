...
Call a V triangle short if it is supercritical or has positive-length
intersection with an adjacent radial arm. If an exact-$M_0$ CE1/CE2 branch
contains at least three short V triangles, then the seven triangles do not cover
the full skeleton $S$.
\end{theorem}
...
C triangle cap for an original positive-support triangle. Every remaining triangle
...
\item CE2, $N_+=1$, with a Vd2 V triangle containing a neighboring midpoint;
...
midpoint $M_s$ is not covered by that V triangle, and the exact-$M_0$ center does not
cover it. Diameter locality leaves only V triangles $s-1,s,s+1$ as possible vertex
triangles containing $M_s$. Thus one of the adjacent V triangles contains $M_s$ and,
...
adjacent exceptional triangle is nonsupercritical and distinct from the two supercritical V triangles.
