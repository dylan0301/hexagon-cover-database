...
The free strict-supercritical envelope says that a strict supercritical V triangle
...
\[
 h\ge B_{\rm sc}(c).
\]
Coverage of $r_1$ gives the supercritical V triangle radial demand at least $c$, so
...
branch and to the normalized Vd1 V triangle containing the neighboring midpoint.
\end{corollary}
...
Assume a reduced CE2 exact-$M_0$ candidate has $N_+=1$, V triangle $T_0$ uniquely
...
\[
 T<\frac12\min\{x,y\}.
\]
Let $(a_0,b_0)$ be the reaches of the unique supercritical V triangle.  Then
\[
...
positive-support V triangle, every placement is impossible directly or reduces to
the all-Vd0 skeleton-data theorem.
\end{proposition}

\begin{proof}
If the supercritical V triangle is $T_0$, an adjacent Vd1/Vd2 V triangle is eliminated by
...
Vd1/Vd2 V triangle is $T_0$, midpoint forcing makes the supercritical V triangle adjacent.
...
special V triangle is $T_0$, midpoint forcing again makes them adjacent.  A Vd2
neighbor rescue is impossible, while a Vd1--supercritical pair is replaced by
two nonsupercritical Vd0 V triangles as in Lemma~\ref{lem:vd1-pair-replacement}; the
