...
Every radial demand in the one-gap, paired-gap, and returned-demand
...
\[
 ...
 T(c,x)=g_c^\vee(x).
\]
For the endpoint records below,
\[
 \mathrm{Cap}(\mathbf e)
...
\[
 ...
 u_\star=1+\beta-\tau.
\]
Every support-isolated endpoint record has $C_5>1/2$.  If
$C_1\le1/2$, the right endpoint uses the direct Full bound $1-A_1$.  If
$C_1>1/2$, the record determines a point of one cell in
...
The left endpoint demand is $C_5=1-a/w>1/2$ by
...
$C_1\le1/2$; this is exactly the existing right-Full row of the complete
$407X$ audit.  When $C_1>1/2$, the raw high-radial map supplies one of the
...
\[
 ...
 =\Psi_{\rm T3}(\xi).
\]
The complementary branch $C_1\le1/2$ is already the direct right-Full case
and is not part of the optimization domain.
\end{proposition}

\begin{proof}
The variables \(A_1,A_5\) are the far-side residual boundary demands, while
\(C_1,C_5\) are the complementary radial demands.  On the high-radial branch,
