\subsection{Cyclic Propagation in the All-$\Vdzero$ Cases}
\label{sec:cyclic-all-vd0}

\subsubsection{The exactly-one-supercritical all-$\Vdzero$ branch}

For a center interval, call the nonempty closed set missed by its two open
endpoint triangles a \emph{V-gap}.  It may be a singleton.  If the maximal
...
Assume that every vertex triangle is $\Vdzero$, $N_+=1$, and the center triangle is
CE1.  If its boundary trace contains a V-gap, the triangles do not cover $H$.
\end{proposition}
...
\end{equation}
Proposition~\ref{prop:signed-one-gap-interface} contains the five actual-vertex triangle
handoffs, the three duality steps, and the final diameter comparison.
...
the two input comparisons used at vertex triangle $4$.  Since $D=R-X$, the inequality
...
upper linear ranges would require $H\le A$ and $H\ge1-A$, respectively, so
both are impossible.  On the the constant or $Q_-$ branch range,
\[
 F_{1-A}(H)\le e(A),
\]
and therefore
...
$Q_+$ range remains.  Its selector gives
\[
 H\le e(A)\le2A+5A^2.
\]
...
Put $p_1=G_{1-A}(H)$.  If vertex triangle $3$ is not on the $Q_+$ branch, then
$p_1\le A+e(A)<3A+5A^2<29/64<1/2$, and
$2R(H-m_3)=\eta+D-A>\eta-P>0$.  Extensivity gives
$p_1\ge H>m_3$, while $p_1<1/2<1-m_3$; thus both linear ranges are impossible.
On the constant, $Q_-$, or the low-$c$ tie one has $F_{1-m_3}(p_1)\le p_1$, so
\[
 G_{1-m_3}(p_1)\ge1-p_1>1-X.
\]
Extensivity of the final reverse map $G_{1-D}$ preserves this strict bound
and proves \eqref{eq:ce1-suffix}.  It remains to treat the selected vertex triangle-$3$
$Q_+$ branch.  Proposition~\ref{prop:reader-universal-tplus} proves once
for all that every selected $Q_+$ arc is increasing and strictly concave.
...
\]
This covers vertex triangle $4$, because its parameter is $d=A<P\le\mu$.  For the
vertex triangle-$3$ parameter $d=m_3$, which need not be at most $\mu$, the remaining
...
$(5-\sqrt{10})/15$ is less than $1/8$.  In the low-$c$ vertex triangle-$3$ range
...
$1-5d<1/3$.
For $d\ge1/5$, extensivity alone is stronger than the vertex triangle-$3$ estimate.
...
\[
 S(R,\mathrm{Const})=-\frac{R q_3(R)}{(25R-4)^2},
\]
where
\[
...
Lemma~\ref{lem:reader-threshold-routing}, applied to the vertex triangle-$2$ map, gives
\[
...
\]
This is \eqref{eq:ce1-suffix}.  The actual-vertex triangle and final inequalities in
...
If the center triangle is CE1 or CE2, every vertex triangle is $\Vdzero$, and
$N_+=1$, then no cover of $H$ can have a boundary gap in the actual open
vertex-triangle traces.
\end{proposition}

\begin{proof}
The midpoint argument used above makes $T_0$ uniquely supercritical.  In an
assumed cover, every boundary gap in the vertex-triangle traces must lie in a
