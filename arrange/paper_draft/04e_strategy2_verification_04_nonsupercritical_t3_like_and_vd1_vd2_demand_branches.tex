...
\begin{proposition}[Exactly one T3-like vertex triangle]
\label{prop:nplus-one-one-t3}
Assume the center triangle is CE1 or CE2, $N_+=1$, exactly one vertex triangle is
T3-like, and no triangle is Vd1 or Vd2.  Then the triangles do not cover $H$.
\end{proposition}

\begin{proof}
Normalize the center midpoint to $M_0$.  A T3-like vertex triangle misses its own
midpoint, a supercritical vertex triangle misses its local midpoint, and a Vd0 vertex triangle cannot
...
\]
and $T_2,\ldots,T_5$ are nonsupercritical Vd0 vertex triangles.  The rational local
...
package, not to scale.  The center triangle and three representative Vd0 vertex
...
Let the center triangle be CE1 or CE2. Assume all six vertex triangles are Vd0,
\[
...
$c_i^C=1-d_i^C$. Since every vertex triangle is Vd0, no adjacent vertex triangle has
a positive-length trace on $r_i$; diameter locality excludes nonlocal triangles.
...
\[
 B_i>1-A_{i+1}.
\]
Nonsupercriticality gives $1-B_i\ge A_i$, while open boundary coverage
gives
\[
 A_{i+1}>1-B_i\ge A_i,
\]
...
\[
 ...
 +g_{1-\delta/R}(1-q)<1.
\]
The three internal vertex triangles $T_2,T_3,T_4$ lie on a center-free boundary
...
We next treat the additional T3-like vertex triangles in the $N_+=0$ branch.  The
...
half-arms if no other triangle has positive support there.
\end{lemma}
...
Assume $T_C$ is CE1 or CE2, $N_+=0$, no triangle is Vd1 or Vd2, and, after
normalization and reflection, $T_0$ is T3-like with selected adjacent
support on $r_1$.  Then the two endpoint vertex triangles in the isolated configuration
satisfy
\begin{equation}
...
$c_1^\ast,c_5^\ast$ the exact radial lower bounds defined below.
\end{lemma}
...
\mathrm{Const}&\mathrm{Const},Q_-&F_{c_5^\ast}(a_5^\ast)+F_{c_1^\ast}(a_1^\ast)<1\\
\mathrm{Const}&Q_+&e(\gamma_5)<a_1^\ast\\
Q_-&\mathrm{Const},Q_-&F_{c_5^\ast}(a_5^\ast)+F_{c_1^\ast}(a_1^\ast)<1\\
Q_-&\mathrm{Lin},Q_+&F_{c_5^\ast}(a_5^\ast)<a_1^\ast\\
...
possibilities for $\Theta$, prove every first-constant entry.  For a first $Q_-$
...
$c=1-y$, $q=tc$, and let $\widehat b_1=F_{c_1^\ast}(q)$.  The selected $Q_-$ equations
give
\[
...
Assume that $T_C$ is CE1 or CE2, $N_+=0$, no vertex triangle is Vd1 or Vd2,
and at least one vertex triangle is T3-like.  Then the seven triangles do not cover
$H$.
\end{proposition}
...
indices, because a T3-like triangle misses its own midpoint and Vd0 triangles cannot
cover adjacent midpoints.  Its first two triangles therefore form a crossed pair
...
Vd1/Vd2 triangle, the short-triangle identity gives $q=k$, where $k$ is the number
of T3-like triangles.  If $k\ge3$, Theorem~\ref{thm:three-short-roles} gives an
...
The actual forward reaches of $T_1,T_5$ are bounded by the two safe maps in
Lemma~\ref{lem:407-four-label-loss}; an extra adjacent-ray reach lower bound on a
possible T3-like $T_1$ only shrinks its feasible set.  Thus their sum is
less than one.  vertex triangles $T_2,T_3,T_4$ must cover more than three units of the
...
$H=\sqrt{1-\beta+\beta^2}\le1$; for linear take $H=1$.  Hence
\[
...
We now assume CE2, $N_+=1$, and exactly one vertex triangle is Vd1 or Vd2.  The corner
...
Assume an original open-triangle skeleton cover in the CE2 exact-$M_0$ branch,
with $T_0$ the unique Vd1 vertex triangle, $M_1\in T_0$, $T_1$ uniquely supercritical,
$T_2,\ldots,T_5$ nonsupercritical Vd0, and no other positive-adjacent-support
triangle.  Then the perimeter together with $r_1$ is not covered.
\end{lemma}
...
Boundary-reach propagation proves the following exclusions:
\begin{enumerate}
\item CE1 or CE2, $N_+=0$, all vertex triangles Vd0;
\item CE1 or CE2, $N_+=0$, with at least one T3-like triangle and no Vd1/Vd2;
\item CE1 or CE2, $N_+=1$, all vertex triangles Vd0, with at least one boundary
gap;
