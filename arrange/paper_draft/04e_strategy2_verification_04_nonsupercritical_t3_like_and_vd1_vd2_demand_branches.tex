...
% STATEMENTS-ONLY-REPLACE-BEGIN: strategy2-t3-package-introduction
Figure~\ref{fig:ce1-ce2-n0-all-vd0} gives a common schematic for the package
proved below.
% STATEMENTS-ONLY-REPLACE-END: strategy2-t3-package-introduction

\begin{figure}[H]
\centering
\includegraphics[width=.78\linewidth]
  {figures/strategy2_ce1_ce2_n0_all_vd0.png}
% STATEMENTS-ONLY-REPLACE-BEGIN: strategy2-package-figure-caption
...
visual inspection.}
% STATEMENTS-ONLY-REPLACE-END: strategy2-package-figure-caption
...
boundary gap. Singleton boundary gaps are included. The center classification
gives $N_{\mathrm{gap}}\in\{0,1,2\}$, with $N_{\mathrm{gap}}\le1$ in CE1.

If $N_{\mathrm{gap}}=0$, the six open vertex traces cover every boundary edge.
Thus
\[
 B_i>1-A_{i+1}.
\]
...
% STATEMENTS-ONLY-REPLACE-BEGIN: boundary-propagation-result-wording
Boundary-reach propagation proves the following exclusions:
% STATEMENTS-ONLY-REPLACE-END: boundary-propagation-result-wording
\begin{enumerate}
