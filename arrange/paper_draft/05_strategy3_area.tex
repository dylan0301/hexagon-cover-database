...
\end{minipage}
\caption{Schematic views of Strategy~3, not to scale.  The local panel
shows one vertex role with boundary reaches $a,b$, and the global panel
shows the six cyclic roles.  Only the darker portions represent area
...
\begin{equation}
 G(T)\ge G(\widehat T)>G(z,L).                  \label{eq:t3-limit-loss}
\end{equation}
...
\begin{enumerate}
\item at least two selected rows are supercritical; or
\item exactly one selected row is supercritical, every role is
      $\Vdzero$ or $\Tlike$, and at least one role is $\Tlike$.
\end{enumerate}
\end{lemma}

\begin{proof}
Put $m=\min_i x_i$ and $M=\max_i x_i$.  The reflection
$y_i=1-x_{-i-1}$ swaps each row's two coordinates and preserves both
alternatives and the total loss.  Hence assume $m\le1-M$.  Every selected
row coordinate is then at least $m$, so
\eqref{eq:min-square-loss} gives loss at least $m^2$ for every row.

In case (1), choose an ascent out of a minimum plateau,
$x_{p-1}=m<x_p$.  Its row contains the coordinate $1-m$ and loses at least
...
other four rows lose at least $m^2$ each, so the total loss is strictly
larger than
\[
 4m^2+(1-m)^2+\frac14
...
$x_5=m<x_0=M$.  The supercritical row loses at least $(1-m)^2$.
Lemma~\ref{lem:t3-nonsupercritical} shows that a $\Tlike$ row is different
from it, and Theorem~\ref{thm:t3-direct-loss} gives that row loss at least
$2m-4m^2$.  The remaining four rows lose at least $m^2$ each.  Their total
loss is at least
\[
...
supercritical rows.  Lemma~\ref{lem:cyclic-area-loss} makes the six vertex
