...
\begin{enumerate}
\item $N_{\mathrm{gap}}=0$, $N_+\ge2$, with arbitrary center and V triangle
types;
\item $N_{\mathrm{gap}}=0$, $N_+=1$, no triangle is $\Vdone$ or $\Vdtwo$,
and at least one triangle is $\Tlike$.
\end{enumerate}
\end{proposition}

\begin{proof}
By Lemma~\ref{lem:gap-exhaustion}, $N_{\mathrm{gap}}=0$ says exactly that
the six V triangles cover $\partial H$, so
Proposition~\ref{prop:strict-handoffs} applies independently of the center
class.
