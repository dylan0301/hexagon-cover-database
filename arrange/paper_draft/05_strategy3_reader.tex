...
Assume the six V triangles cover $\partial H$.  Choose strict handoff
parameters
\[
...
\begin{enumerate}
\item if at least two V triangles are supercritical, at least two indices
are ascents;
\item if exactly one V triangle, indexed by $p$, is supercritical, then
...
Suppose first that there are at least two supercritical V triangles.  By
...
If there is exactly one supercritical V triangle,
...
\]
In either case the six V triangles have total area inside $H$ strictly
below five; the C triangle has area one.
...
\begin{proof}
For CE0 the six V triangles cover $\partial H$, so
