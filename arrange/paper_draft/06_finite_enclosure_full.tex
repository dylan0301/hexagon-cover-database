...
formula. The fixed-start Newton inner reduction moves the two outer frontier
...
points, constructed from the fixed parameter $k=1/2$, reduce the enclosure to four supporting contacts: the lines through
two consecutive points and the two exposed disk tangencies. The line bounds
are analytic; the paired tangent residuals use explicit low-degree comparisons with exact
coefficient bounds and simultaneous radius transfer.  The complete calculation is
