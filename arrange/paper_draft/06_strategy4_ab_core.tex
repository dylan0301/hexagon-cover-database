\section{Strategy 4: the zero-gap $AB$-set residual core}
\label{sec:ab-core}
...
exact finite semialgebraic condition of degree at most four.  We shall also
use the immediate antitonicity
\begin{equation}
 a'\ge a,\quad b'\ge b
 \quad\Longrightarrow\quad
 \mathcal R(a',b')\subseteq\mathcal R(a,b).    \label{eq:ab-antitone}
\end{equation}
...
For each hexagon vertex $V_i$, let $R_i(a,b)$ denote the rotated copy of
$\mathcal R(a,b)$ in its corner cone, with the first coordinate incoming and
the second outgoing.  Formula \eqref{eq:ab-antitone} is valid for every
$R_i$.

\subsection{From zero gaps to two model cores}

\begin{lemma}[The extreme jump]
...
\begin{align}
 \mathcal U_6^\circ
 &=\interior(H)\setminus\bigcup_{i=0}^5\interior R_i(a_i,b_i),
                                                        \label{eq:actual-core}\\
 \mathcal C_{\rm asym}(a,b)
 &=\interior(H)\setminus\left(
 \interior R_4(a,b)\cup\bigcup_{i\ne4}\interior R_i(p,q)\right),
                                                        \label{eq:asym-core}\\
 \mathcal C_{\rm sym}(a,b)
 &=\interior(H)\setminus\bigcup_{i=0}^5\interior R_i(p,q).
                                                        \label{eq:sym-core}
\end{align}
If $P\in\interior(H)$ is covered by an open vertex role, then a small ball
about $P$ lies both in its corner cone and in its closed associated triangle;
hence $P$ lies in the plane interior of its row union.  Thus a full cover
forces
\[
 \mathcal U_6^\circ\subseteq U_C.
\]
Lemma~\ref{lem:ab-extreme-jump} and antitonicity give
\begin{equation}
 \boxed{
 \mathcal C_{\rm sym}(a,b)
 \subseteq\mathcal C_{\rm asym}(a,b)
 \subseteq\mathcal U_6^\circ
 \subseteq U_C.}                                \label{eq:core-chain}
\end{equation}

\subsection{The forced disk and the three asymmetric witnesses}
...
\begin{lemma}[Symmetric radial witnesses]
...
For every $k$,
\[
 P_k=(1-c_*)V_k\in\mathcal C_{\rm sym}(a,b).
\]
Consequently every convex set containing $\mathcal C_{\rm sym}(a,b)$
contains the centered closed disk
\begin{equation}
...
We need only the following consequence of the neighboring-ray formula in
Theorem~\ref{thm:neighbor-ray-capacity}.  If $0<m\le M$ and $m+M\le1$, then
\begin{equation}
 C_+(M,m)=1-m,\qquad C_+(m,M)\le1-m.             \label{eq:neighbor-consequence}
\end{equation}
For reference, when $M\le1/2$ this follows by evaluating the defining cubic
\[
 f_M(z)=z^3-(M+2)z^2+2(M+1)z-1
\]
at $z=1-m$: $f_M(1-m)\ge m^2(1-2m)\ge0$.
The same evaluation with $m$ in place of $M$ handles the first two pieces of
$C_+(m,M)$.  On its final piece,
\[
 C_+(m,M)=m+\frac12-\sqrt{(m+M)^2-\frac34}.
\]
If $m\le1/4$, this is at most $m+1/2\le1-m$.  If $m>1/4$, let
$\pi=\pi(m)$ be the root of $f_m$ in $[m,1]$ and put $d=\pi-m$.
Then $0\le d\le1-2m$ and the root equation is
\[
 d\pi(2-\pi)=2\pi-1.
\]
The final branch begins after
$\tau(m)=1-m-d(1-\pi)$, and the preceding identity gives
\[
 (m+\tau(m))^2-(4m^2-2m+1)
 =(d-2m)(d+2m-1)\ge0.
\]
Indeed $m>1/4$ implies $d\le1-2m<2m$, so both factors are nonpositive.
Thus $M>\tau(m)$ gives $(m+M)^2>4m^2-2m+1$, and hence
$C_+(m,M)<1-m$.
Together with \eqref{eq:cstar-bounds},
\begin{equation}
 c_*>\max\{C_+(p,q),C_+(q,p)\}.                 \label{eq:neighbor-strict}
\end{equation}

It suffices to prove the assertion for $P_0$.  In the local cone coordinates
at $V_j$, its coordinates and squared distance from $V_j$ are
\[
\begin{array}{c|c|c}
j& P_0&\lVert P_0-V_j\rVert^2\\ \hline
0&(c_*,c_*)&c_*^2\\
1&(c_*,1)&c_*^2-c_*+1\\
2&(1,2-c_*)&c_*^2-3c_*+3\\
3&(2-c_*,2-c_*)&(2-c_*)^2\\
...
\end{array}
\]
By definition of the radial maximum, $P_0$ lies on, but not in the plane
interior of, $R_0(p,q)$.  The diameter bound excludes it from
$R_2,R_3,R_4$.  Membership in $R_1$ or $R_5$ would contradict
\eqref{eq:neighbor-strict}.  Hence $P_0\in\mathcal C_{\rm sym}$, and rotation
gives all six points.  Their convex hull is a regular hexagon of circumradius
$1-c_*$, whose incircle is exactly \eqref{eq:forced-disk}.
\end{proof}
...
\begin{lemma}[Fixed-line sign lemma]
...
\]
and the reflected bound holds for $L_-(\lambda_-)$.  Each first-root point
lies outside the opposite comparison circle, while $J$ lies strictly outside
both comparison circles.
\end{lemma}
...
multiplication by a positive factor, has numerator
\[
...
$P_0\ge9+54U-18U>0$.  Hence $N_0>0$.  Since the restriction of $F$ to
$L_-$ is a convex quadratic, it is positive from the junction to the line
endpoint and has no intersection there.
...
$D^2\ge3U(2+U)$ and $(2+U)^3>3U$.  Hence
$J_u+J_v<1<3-2a$, and the coordinate sum is affine on the
line segment, so the same strict bound holds at the first root.  Reflection
gives the bound on $L_-$.  The convexity and signs above also give all
same- and opposite-circle assertions.
\end{proof}
...
\begin{lemma}[Asymmetric witness membership]
\label{lem:asymmetric-core-witness}
The three points $Q_-,Q_0,Q_+$ lie in
$\mathcal C_{\rm asym}(a,b)$.
\end{lemma}

\begin{proof}
The root order and positivity of the line coordinates give
$0<u,v<1$ for all three points.  Each lies on the frontier of a unit
triangle containing $V_4$, so $\mathsf q(u,v)\le1$; hence
...
\[
 ...
 \end{array}
\]
Every $R_i(p,q)$ is contained in the unit disk about its mandatory point.
For $P=(u,v)$,
\[
...
\]
The inequalities $J_u,J_v<1/2$ and the line order exclude the appropriate
two witnesses directly.  For the remaining pair, the first-root circle and
the sum bound in Lemma~\ref{lem:fixed-line-signs} give
\[
 \lVert Q_+-(2,1)\rVert^2-1
 =a(3-a-(Q_+)_u-(Q_+)_v)>a^2,
\]
and the reflected inequality for $Q_-$.  All three points are farther than
one from $(2,2)$, because writing $u=1-\xi$, $v=1-\eta$ gives
\[
 \lVert P-(2,2)\rVert^2
 =1+\xi+\eta+\xi^2+\eta^2-\xi\eta>1.
\]
The circle signs in Lemma~\ref{lem:fixed-line-signs} exclude the interiors of
$R_3$ and $R_5$.  Finally all three points lie on the exact line frontier of
$R_4(a,b)$, and hence not in its interior.  This proves the claim.
\end{proof}

By \eqref{eq:core-chain}, Lemmas~\ref{lem:symmetric-core-witness} and
\ref{lem:asymmetric-core-witness} force the compact set
\begin{equation}
 K_{\rm wit}(a,b)=
...
\[
 \Delta=\operatorname{cross}(C,RA),\qquad
 u=\langle C,RA\rangle,\qquad
 Z_X=\tau\lVert X\rVert^2-\eta u,
\]
\begin{equation}
 P_X=(\lVert X\rVert^2-\eta^2)\Delta^2-Z_X^2,
 \qquad X\in\{A,C\}.                            \label{eq:mixed-P}
\end{equation}
To compare these quantities with the certificate, represent global vectors
as $(x,hy)$, put $C'=R^{-1}C$, and use the notation
$\Delta_0,r_X^0,\ell_X^0,P_X^0$ of
\eqref{eq:mixed-certificate-quantities}.  Rotation invariance and
$\eta=h(1-c_*)$, $\tau=h(2c_*-1)$ give the exact scaling
\begin{equation}
 \Delta=-h\Delta_0,\qquad
 Z_X=-h\ell_X^0,\qquad
 P_X=h^2P_X^0,                                  \label{eq:mixed-scaling}
\end{equation}
and $r_X^0=\lVert X\rVert^2-\eta^2$.
The outward-rounded certificate in
Theorem~\ref{thm:cert-mixed-overlap} proves, on the complete strict domain,
that the two radicands are nonnegative, $\Delta_0\le0$, and
$P_A,P_C\ge0$; the ray-order calculation above makes $\Delta>0$.
Consequently
\[
 \sqrt{\lVert X\rVert^2-\eta^2}\,\Delta\ge Z_X,
 \qquad X=A,C.
\]
For $X=A$ this is exactly the common-support inequality for the caps of
$\mathbb D(C,2\eta)$ and $\mathbb D(W,\eta)$; for $X=C$ it is the analogous
inequality for $\mathbb D(W,\eta)$ and $\mathbb D(RA,2\eta)$.
...
\begin{theorem}[Center-independent zero-gap obstruction]
...
\begin{proof}
The extreme-jump construction gives a parameter pair in
\eqref{eq:ab-strict-domain}, and \eqref{eq:core-chain} together with the two
witness lemmas gives the compact containment
$K_{\rm wit}(a,b)\subset U_C$.  Theorem~\ref{thm:witness-enclosure} says
...
For a distinguished selected ascent $x_3<x_4$, the five antitone comparisons
used in \eqref{eq:core-chain} hold exactly when $x_3$ is a global minimum and
$x_4$ a global maximum.  The exact-one chain
\eqref{eq:extreme-chain} guarantees this.  With two or more ascents there
need not be any minimum-to-maximum ascent, so replacing all other rows by
$R_i(1-x_4,x_3)$ is not justified.  The $N_+\ge2$ branch is therefore handled
by Strategy~3 or Strategy~1, not by this comparison.
\end{remark}

\begin{proposition}[Branches closed by the residual core]
