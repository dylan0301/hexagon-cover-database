...
that all six V triangles are $\Vdzero$, that they cover the full boundary,
and that exactly one actual boundary V triangle is supercritical.  It therefore
applies to every $N_{\mathrm{gap}}=0$ case with these V triangle data,
independently of whether the center is $\CEzero$, $\CEone$, or $\CEtwo$.
Six common radial points and three asymmetric points
are excluded directly from all six actual V triangles and are therefore
forced into the C triangle.  Only the unique supercritical V triangle uses an
