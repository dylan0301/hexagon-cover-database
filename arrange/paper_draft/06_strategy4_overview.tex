...
\begin{figure}[H]
\centering
\input{figures/strategy4_nine_point_witness}
\caption{Schematic Strategy~4 witness, not to scale.  The six radial points
are symbolically exact; the locations of $Q_-,Q_0,Q_+$ vary with $(a,b)$.
No exclusion or enclosure inequality is inferred from the picture.}
\label{fig:nine-point-witness}
\end{figure}
...
\begin{figure}[H]
\centering
\input{figures/strategy4_cap_chain}
\caption{Schematic support-cap chain, not to scale.  The drawing explains
the cyclic covering mechanism only.  The overlap inequalities are proved in
Appendices~\ref{app:strategy4-details} and
\ref{app:exact-mixed-overlap}.}
\label{fig:cap-chain}
\end{figure}
