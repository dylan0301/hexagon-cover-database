...
This appendix solves the coordinate problems used by the
geometric interfaces in Section~\ref{sec:structure-common}.  The shared equilateral-enclosure calculus is proved in the main text,
Subsection~\ref{sec:universal-calculus}. Each calculation here is attached
to the body statement it supplies.
...
The calculation uses the local wedge and the two orientation charts just
introduced.  Its output is a closed majorant, not a replacement open triangle.
...
The enclosure gauge, support-cell rotation, and finite calipers are
stated and proved in Subsection~\ref{sec:universal-calculus} of the
finite-enclosure chapter. The local optimization below applies those
general results. For these reminders, write
\[
 W_K(n):=\sum_{j=0}^{2}h_K(\mathsf R^j n)\qquad(\|n\|=1).
\]
For reference, the gauge formulas used here are
\[
 \Lambda(K):=\frac{2}{\sqrt3}\min_{\|n\|=1}W_K(n),
\]
\[
 K\text{ is contained in a unit C triangle}
 \iff
 \min_{\|n\|=1}W_K(n)\le\frac{\sqrt3}{2}.
\]
For the disk--finite-set applications, use the same inverse scaling:
\[
 \Lambda(K)=\frac{2}{\sqrt3}\min_{\|n\|=1}W_K(n),
 \qquad K=\mathbb B(O,\rho)\cup\{w_1,\ldots,w_s\}.
\]
These recall the main-text proposition, whose proof is not repeated.
...
Lemma~\ref{lem:support-cell-rotation} reduces its least enclosure side
...
\subsection{Self-midpoint corollary}

\begin{lemma}[Self-midpoint admissibility corollary]
