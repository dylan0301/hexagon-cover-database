\section{Exact Polynomial Positivity Certificate}
\label{sec:strategy4-exact-certificate}

The two mixed cap overlaps in the finite-enclosure method are certified by exact symbolic
...
The certificate has a machine-readable provenance manifest.  The file
\nolinkurl{certificate_provenance.json}, in the numbered $3105X$ computation
directory, identifies the six sparse-data shards and both exact verifiers by
full Git blob and records their roles.  They decode to one canonical
sparse-polynomial transcript with
SHA--256 digest
\begin{verbatim}
...
Retain the notation of \zcref{sec:ab-core}.  Thus
\[
...
\[
 ...
 c_*=c_{\max}(p,q).
\]
For the certificate formulas only, abbreviate
\[
 A:=P_-,\qquad C:=P_+,\qquad \eta:=r_*,
 \qquad I(X,r):=\operatorname{Cap}(X,r).
\]
...
If $Q_A,Q_C\ge0$, then
\[
 I(C,2\bar\eta)\cap I(C+\mathsf R A,\bar\eta)\ne\varnothing,
\]
and
\[
 I(C+\mathsf R A,\bar\eta)\cap I(\mathsf R A,2\bar\eta)\ne\varnothing.
\]
...
\zcref{prop:technical-four-overlaps} hold.
\end{theorem}
...
reflection supplies $z\le0$.  \zcref{lem:paper-residual-to-overlap} then
