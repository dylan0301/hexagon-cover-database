...
\input{fixed_witness/optional_B}
...
The CE1 part of BC uses a boundary tail floor at $T_4$, three middle
handoffs, and the transverse radial reaches at $T_4,T_3,T_2$. The old
five-handoff return is a corollary, not an input to BC.
...
All reaches below are actual. Assume
\[
 A_1\ge X,\qquad B_i+A_{i+1}\ge1\quad(1\le i\le3),
 \qquad B_4\ge h_0,
\]
and
\begin{equation}
...
\end{equation}
These hypotheses are incompatible. Neither a fifth handoff nor a bound
on $A_0$ is part of this local statement.
\end{proposition}
...
Start directly from the supplied tail floor
\begin{equation}
 \boxed{B_4\ge h_0.}
 \label{eq:ce1-b4-floor}
\end{equation}
The first step below uses only this floor, the boundary handoffs, and the
radial hypothesis at $T_4$, so it remains available before recovering the
third radial demand.
...
\begin{corollary}[Retained five-handoff return]
\label{cor:ce1-five-handoff-return}
Under the same signed CE1 domain, let $T_0$ be a V triangle and let
$T_1,\ldots,T_5$ be nonsupercritical. Assume $A_1\ge X$,
$B_i+A_{i+1}\ge1$ for $1\le i\le4$, $B_5+A_0\ge1$, and all three
radial hypotheses of \zcref{eq:ce1-radial-hypotheses}. Then
\begin{equation}
 \boxed{A_0>1-h_0.}\label{eq:ce1-return-bound}
\end{equation}
\end{corollary}
\begin{proof}
Otherwise
\begin{equation}
 A_0\le1-h_0.\label{eq:ce1-return-negation}
\end{equation}
The final handoff gives $B_5\ge h_0$. Nonsupercriticality of $T_5$
and the preceding handoff imply $B_4\ge B_5\ge h_0$, so
\zcref{prop:new-ce1-direct-certificate} gives a contradiction. Only
this optional corollary retains the fifth handoff.
\end{proof}
...
$\widehat P_i$ of the active BC theorem. Their Vd0-specific forcing is
