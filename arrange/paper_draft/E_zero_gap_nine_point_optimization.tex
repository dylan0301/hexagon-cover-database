...
This appendix supplies exact frontier and radial calculations, point
exclusions, Newton reduction, and the four-contact enclosure proof for the
type-independent zero-gap obstruction. The radial step reuses the common $C_+,C_-$ forcing interface;
...
These four normals and their $120^\circ$ rotations partition the orientation
circle into cells with fixed support sources. On each open cell,
\[
 \Psi(n(\theta))=k\eta+\langle v,n(\theta)\rangle.
\]
If a point is active, its projection exceeds $\eta$, so
$\Psi''=-\langle v,n(\theta)\rangle<0$. A minimum is attained on a cell
boundary. On a disk-only cell the sum is constant, and its minimum is also
attained at a boundary; such a boundary exists since $\|A\|>\eta$.
This is the disk--finite-set caliper argument of
\zcref{prop:new-disk-finite-caliper}, with inactive ties removed by the hull
geometry. Rotation invariance leaves just the four displayed normals.
\end{proof}
...
\zcref{thm:paper-exact-mixed-certificate} proves the unchanged eight
...
\]
The Newton inner points and exact algebraic certificate are retained;
only the terminal geometry has been replaced by supporting contacts.
\end{proof}
