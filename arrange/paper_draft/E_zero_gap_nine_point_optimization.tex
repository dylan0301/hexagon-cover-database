...
This appendix supplies the exact frontier, radial, pointwise, Newton, and
support-arc calculations for the type-independent zero-gap nine-point
obstruction. The radial step reuses the common $C_+,C_-$ forcing interface;
the three asymmetric witnesses and the exact certificate are unchanged.  It uses
the public parameters and witness \(K_{\rm wit}(a,b)\) from
\zcref{sec:strategy4-reader}.  The symbols \(A,B,C\) introduced below are
appendix-only Newton inner points required by the exact certificate; they do
not replace the public frontier points \(Q_-,Q_0,Q_+\).

The finite caliper criterion used here is the shared result
\zcref{thm:cert-caliper} in Appendix A.  It is not rederived in this
appendix.

\subsection{Corner-clipped $AB$-unions}

Apply the rigid motion that sends the supercritical corner $V_4$ to the
local origin and its two incident edge directions to $e_A,e_B$.  Within this
calculation $O,A,B,C$ denote the local origin, anchors, and corner cone; the
feasible union keeps the public symbol $\mathcal R_{AB}(a,b)$.
...
By \zcref{lem:ab-extreme-jump}, every selected pair dominates $(p,q)$.
The common-pair forcing corollary
\zcref{cor:new-uniform-common-pair-forcing} therefore forces the same
$D_i=(1-c_*)V_i$ for every V-type pattern. To make the adjacent-role step
explicit, \zcref{lem:new-common-pair-domination} and
\eqref{eq:cstar-bounds} give
\[
 C_+(p,q),C_-(p,q)\le1-m<c_*.
\]
A neighboring open V triangle containing $D_i$ would have positive trace
on $r_i$ and its closure would realize neighboring coordinate $c_*$ with
the common anchors, contradicting this inequality. If its type forbids
that trace, openness already gives a contradiction. The own-role and
nonlocal exclusions are the shared type-aware forcing lemma. Thus no
V triangle contains $D_i$, and full coverage puts it in $U_C$.
...
$0<u,v<1$ for all three points.  Each belongs to $\mathcal R_{AB}(b,a)$ and hence
...
$\mathcal R_{AB}(b,a)$, since it contains $V_4,X_3,X_4$.  All three witnesses lie
...
$\mathcal R_{AB}(b,a)$.  This contradiction excludes $U_4$.
...
\begin{equation}
 A,\ B,\ C,\ W,\ RA                              \label{eq:cap-ray-order}
\end{equation}
...
It remains to check the two mixed overlaps.  The exact rational envelopes, Gram reductions, and global positive-basis identities are recorded once in Appendix~\ref{sec:strategy4-exact-certificate}. Theorem~\ref{thm:paper-exact-mixed-certificate} gives precisely the overlaps for
$\mathbb D(C,2\eta)$ with $\mathbb D(W,\eta)$ and for
$\mathbb D(W,\eta)$ with $\mathbb D(RA,2\eta)$.
...
\subsection{Newton reduction in the body notation}

The containment established by
\zcref{lem:technical-newton-reduction} is recorded under the public routing
label
\begin{equation}
 \widehat K=\conv\bigl(\mathbb B(O,\eta)\cup\{A,B,C\}\bigr)
 \subseteq\conv(K_{\rm wit}(a,b)),
 \qquad
...
\begin{lemma}[Solved witness-enclosure optimization]
\label{lem:appendix-witness-enclosure-calculation}
For every parameter pair in \zcref{eq:reader-ab-domain},
\[
...
\[
 \Lambda(K_{\rm wit}(a,b))\ge3(1-c_*),
\]
which proves the claim when \(c_*\le2/3\).  Assume \(c_*>2/3\).
By \zcref{eq:reader-newton-reduction}, it is enough to enclose
\(\widehat K\).  The Minkowski sum
\[
 \widehat K+\mathsf R\widehat K+\mathsf R^2\widehat K
\]
contains the full rotation orbits of the four disks in
\zcref{lem:reader-cap-chain}.  The ray order, radius bounds, and equal-radius
intersections are \zcref{prop:technical-four-overlaps}; the two mixed
intersections are \zcref{thm:paper-exact-mixed-certificate}.  Therefore
\zcref{lem:reader-cap-chain} gives \(\Lambda(\widehat K)\ge1\), and
monotonicity gives the result.
\end{proof}
