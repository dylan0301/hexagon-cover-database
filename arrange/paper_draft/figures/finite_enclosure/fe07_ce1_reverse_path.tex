...
\begin{tikzpicture}[
  ...
  \node[state] (a0) at (0,1.35) {$B_0>2h_0$};
  \node[state] (b5) at (3.05,1.35) {$B_5>B_0/2>h_0$};
  ...
  \draw[arr] (a0)--node[above=3pt,figlabel,fill=white,inner sep=.6pt] {shared gap anchor} (b5);
  \draw[arr] (b5)--node[above=3pt,figlabel,fill=white,inner sep=.6pt] {path monotonicity} (b4);
  \draw[harr] (b4)--node[above=3pt,figlabel,fill=white,inner sep=.6pt] {selected $\mathcal Q_+$ at $T_4$} (b3);
  \draw[harr] (b3.east)--++(.55,0)--++(0,-1.10)--
    node[right=2pt,figlabel,align=left,fill=white,inner sep=.6pt] {selected $\mathcal Q_+$ at $T_3$\\and the scalar estimate}
    ++(-10.25,0)--(b2.west);
...
proof starts from the BC tail input, moves through the
actual boundary reaches, uses two selected-arc estimates and one
