...
\begin{tikzpicture}[scale=3.0,>=Latex]
  ...
  \node[figlabel,right] at (D0) {$P_0^{\rm rad}$};
  \node[figlabel,above right] at (D1) {$P_1^{\rm rad}$};
  \node[figlabel,below left] at (D4) {$P_4^{\rm rad}$};
  ...
  \node[figlabel,atlasbox,align=left,anchor=west] at (1.20,.55)
    {six symmetric points: $P_i^{\rm rad}=(1-c_*)V_i$\\
     ...
     $\mathbb B(O,r_*)\subset\operatorname{conv}\{P_i^{\rm rad}\}$};
  \draw[flow] (1.15,.30)--(.34,.05);

  \node[figlabel,align=center] at (0,-1.22)
    {$K_{\rm wit}=\mathbb B(O,r_*)\cup\{Q_-,Q_0,Q_+\}
      \subset U_C$ under a hypothetical cover};
\end{tikzpicture}%
}
\caption{The type-independent zero-gap nine-point witness.  The six symmetric radial points
