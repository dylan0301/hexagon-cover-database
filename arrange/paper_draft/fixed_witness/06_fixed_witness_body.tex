...
\end{theorem}
\begin{proof}
The path bounds and neighboring-capacity estimates force both radial witnesses,
with the explicit clipping by $A_3$. The proof in
\zcref{lem:appendix-fixed-transverse} then uses the five-point caliper theorem
or a diameter obstruction. It does not normalize an enclosing center triangle.
\end{proof}
...
\end{theorem}
\begin{proof}
The cases $a=0$ or $s\ge1$ have a diameter obstruction. Otherwise put
$v=a/s$ and replace the farther boundary point by $Y(v)$ inside the same hull.
The four caliper evaluations in \zcref{lem:appendix-fixed-four-point}
all exceed one. No center normal form is needed.
\end{proof}
