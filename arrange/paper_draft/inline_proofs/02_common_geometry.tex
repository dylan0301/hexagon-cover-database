...
This section supplies three inputs to the later obstructions. First it
identifies and normalizes the triangles without changing their covered
traces. Next it relates boundary reach to radial reach. Finally it shows
how a C triangle reaching the boundary is constrained by the radial
midpoints. The orientation charts support these calculations; they are
not additional cases of the final covering argument.

\subsection{Open covers and distinguished roles}
For a hypothetical cover by seven open unit triangles, write $T=\overline U$
for the closure of each open triangle $U$.
...
\input{reading_guide/coverage_scope}
...
\subsection{C triangle and V triangle incidence}
...
\]
The values $0,1,2$ define the types $\CEzero,\CEone,\CEtwo$, respectively.
...
\]
These are the two nonadjacent-edge configurations.  The distance comparison is strict for both relative-interior parameters.
\end{proof}
...
\begin{lemma}[Shared corner chart and wedge reduction]
\label{lem:shared-corner-chart}
...
\end{lemma}
\begin{proof}
Use \zcref{lem:shared-corner-chart} at $V_0$. In the unit ball,
\[
...
\subsection{Shared orientation charts and exact-trace normalization}
...
\]
In the shared corner chart, physical rotation through $60^\circ$ is
$(x,y)\mapsto(x-y,x)$. After relabelling the vertices, every orientation
...
\end{array}
\]
They cover successive $60^\circ$ direction sectors, including their
common endpoints. The following half-plane descriptions allow closed
containment of the corner; strict interior containment is recorded separately.
Here $U,V$ are affine slack functions, not the open triangles $U_i$.
In Type I write
\begin{equation}
...
diameter-one condition, \zcref{lem:local-wedge} puts it in $H$.  Hence
...
$H$, their open and closed traces, and therefore their maximal reaches, are
unchanged. In particular, the sum of the two boundary reaches is unchanged.
\end{proof}
...
\begin{proof}
\zcref[S]{lem:local-wedge} gives $o\le2$ when $n\ge1$ and $o\le1$ when
$n=2$, while $o\ge1$.  \zcref[S]{prop:vd0-exact-trace-normalization} replaces the only additional raw
pair, $(3,0)$, by $(2,0)$.
\end{proof}

\subsection{Actual reaches and boundary gaps}
...
positive-length C trace.  \zcref[S]{prop:ce-classification} gives the
count and the final equivalence.
\end{proof}

\input{reading_guide/boundary_tradeoff}

\subsection{T3-like trace majorants and midpoint exclusion}
...
\]
where the second comparison is equivalent to $(1-z)^2>0$.
At $M_0=(1/2,1/2)$ the affine coordinate $V$ equals
$\beta-(1-t)/2<0$, so $M_0\notin T_0$.
...
\begin{lemma}[Orientation of a forward Vd1 supplier]
\label{lem:vd1-forward-orientation}
In \zcref{prop:vd-corner-normal-form}, if a Vd1 role contains $M_1$
openly, then $t\ge1$.
\end{lemma}
\begin{proof}
Suppose $t<1$. Open containment of $M_1=(1/2,1)$ gives
$A_0+tB_0<d-1-t/2$. The backward radial interval has endpoints
$\ell_5=(1-A_0)/(t+1)$ and $u_5=d-A_0-tB_0-t$, so
\[
 u_5>1-t/2>\frac1{t+1}>\ell_5,
 \qquad 0<\ell_5<1.
\]
Here $1-t/2-1/(t+1)=t(1-t)/(2(t+1))>0$.
Thus $r_5$ has positive trace as well as $r_1$, contradicting Vd1.
\end{proof}
...
\begin{proof}
In \zcref{prop:vd-corner-normal-form}, reflection sends
\[
 (t,A_0,B_0,M_1,M_5)
...
can be chosen so that one side contains two distinct points of $P$, or is
tangent to the disk and contains one point of $P$.
\end{proposition}
...
\]
At $V_0$, take $u=V_5-V_0$ and $v=V_1-V_0$; translating the
hull by $V_0$ gives the corresponding anchors in the hexagon.
...
\zcref[S]{lem:support-cell-rotation} reduces its least enclosure side
to its four edge normals.  For the edges through $0$ one obtains
\[
 L_{0A}=b+\max\{a,c\},
...
Write $f(a,b):=1-c_{\max}(a,b)$ for $a+b\le1$.

\subsection{Self-midpoint obstruction and outgoing envelopes}
...
\end{lemma}
\begin{proof}
Put $m=\min\{a,b\}$ and $M=\max\{a,b\}$. If the four-point hull
were admissible with $a+b>1$, Cell $S$ of
\zcref{prop:exact-admissible-set} would require $F_S(m,M,1/2)\le0$.
However,
\[
\begin{aligned}
 4F_S(1-M,M,1/2)&=M^2(2M-1)^2\ge0,\\
 \frac{\partial}{\partial m}\bigl(4F_S(m,M,1/2)\bigr)&=2m+4M^2>0.
\end{aligned}
\]
Since $m>1-M$, this is impossible. If $a+b=1$ and the four points
have positive margin, increase the two boundary demands slightly to
...
supercritical triangle has that radial demand.

\begin{lemma}[Strict-supercritical outgoing envelope]
\label{lem:strict-supercritical-envelope}
If a V triangle is supercritical and has own-radial reach at least
$c\in[0,1/2)$, then both boundary reaches are strictly below
$M_c^{\rm sup}$ from \eqref{eq:supercritical-envelope-definition}.
No supercritical triangle has own-radial reach at least $1/2$.
\end{lemma}
\begin{proof}
Let $m$ and $M$ be the smaller and larger actual boundary reaches.
For $c>0$, the selected supercritical cell gives $F_S(m,M,c)\le0$,
whereas $m>1-M$ and
\[
...
\]
Since $M>1/2>c$, these identities imply
$1-2c+cM-M^2>0$. Solving the quadratic gives
$M<(c+\sqrt{c^2-8c+4})/2$.
For $c=0$, each boundary reach is strictly less than one because its
opposite distinguished vertex is excluded from the closed V triangle.
The endpoint $c=1/2$ is excluded by the self-midpoint obstruction.
\end{proof}
...
The signed-center coordinates describe a triangle that contains $O$ in
its interior and has a positive-length boundary trace. Their purpose is to relate its boundary endpoints to
its radial exits, not to classify a new covering configuration. The same
formulas will be applied later to an arbitrary enclosing candidate, with
its parameters recomputed. The midpoint identity and the boundary-length
cap are the geometric conclusions to retain from this calculation.

\label{sec:signed-ce12-reductions}

The CE1 and CE2 cases share one signed cell. These coordinates give
the midpoint property, trace bounds, and later enclosure calculations.
