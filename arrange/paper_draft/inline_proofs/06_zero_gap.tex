...
Here the frontier points have already been forced. The next step makes
only the enclosure lower bound easier to verify: it moves the two outer
points toward the middle point, so the candidate's containment is preserved
by convexity. The tangent intercept from one Newton step gives exact
rational expressions while remaining on the inner side of the first root.
...
\input{reading_guide/certificate_scope}
