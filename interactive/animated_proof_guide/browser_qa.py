...
ap=argparse.ArgumentParser();ap.add_argument('--folder',type=Path,required=True);ap.add_argument('--output',type=Path,required=True);ap.add_argument('--inline',type=Path);ap.add_argument('--executable-path',type=Path);a=ap.parse_args();a.output.mkdir(parents=True,exist_ok=True)
server=ThreadingHTTPServer(('127.0.0.1',0),partial(QuietHandler,directory=str(a.folder)))
threading.Thread(target=server.serve_forever,daemon=True).start()
errors=[];failed=[]
with sync_playwright() as p:
    b=p.chromium.launch(**({'executable_path':str(a.executable_path or '/usr/bin/chromium'),'args':['--no-sandbox']} if a.inline or a.executable_path else {}))
    ...
    if a.inline:page.goto(a.inline.resolve().as_uri(),wait_until='load')
    else:page.goto(f'http://127.0.0.1:{server.server_address[1]}/index.html',wait_until='networkidle')
    assert page.locator('details.case').count()==48
    ...
    page.locator('#search').fill('singleton');assert 0<page.locator('details.case:not([hidden])').count()<48
    ...
    for name in ['bc-build','replacement-minus','area-cyclic','area-max']:
        page.evaluate('(id)=>location.hash=id',name);page.wait_for_timeout(120)
        page.locator('#'+name).screenshot(path=str(a.output/(name+'-card.png')))
    ...
    report={'passed':True,'entries':48,'desktop':[1440,1100],'mobile':[390,844],'tests':['images resolve','manual play/pause','search','chapter filter','hash opens correct case','mobile has no horizontal overflow'],'pageErrors':errors,'failedRequests':failed,'browser':b.version}
