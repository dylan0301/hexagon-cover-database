...
def check(folder=HERE,require_live=False):
    ...
    assert len(records)==39 and len(cases['cards'])==47
    ...
    for s in records.values():
        ...
        assert gif.size==poster.size==(640,770)
        assert gif.n_frames==s['frameCount']==14
        assert gif.info.get('loop')==0
        assert s['canvasDistinctFrames']>=2
        ...
        snap=json.loads((folder/'snapshots'/f'{s["id"]}.json').read_text());assert snap['version']==(8 if s['snapshotKind']=='free' else 11)
        for f in s['frames']:
            m=f['metrics']
    ...
    print('PASS: 47 entries, 39 animated GIFs, 546 rendered states; domains, references, snapshots, contacts, suppliers, replacement, and boundary-only example.')
