...
This is the active terminal-first interface for Strategy 3. Detailed case
files retain the placement calculations; this file states the common forcing
engine, six terminal families, and the adapter-to-terminal map. No active proof
composes boundary-transfer maps.

## 1. Common forcing engine

Suppose the closed V role $T_i=\overline{U_i}$ realizes selected lower bounds
...
[`2004`](../20XX_V_triangle_geometry/2004_admissible_set.md). For an adjacent
role actually permitted by its V type to support $r_i$, use the neighboring
capacities $C_+$ and $C_-$ from
[`2008`](../20XX_V_triangle_geometry/2008_neighbor_ray_max_c_formula.md). Put

$$
\Gamma_i=
\max\{c_{\max}(a_i,b_i),C_+(a_{i-1},b_{i-1}),C_-(a_{i+1},b_{i+1})\},
\tag{1}
$$
...
The type-aware radial-forcing theorem in
...
under a hypothetical cover. For a common pair $p,q\ge0$, $p+q\le1$, put
...
Thus one radius controls the own traces and every permitted T3-like
neighboring trace.

If a compact set $K$ lies in the open unit C triangle, moving its three side
lines inward gives
...
A one-gap state with $N_+=0$, no Vd1/Vd2 role, and only Vd0 or at most two
...
If the intervening V roles dominate $(p,q)$ and every permitted neighboring
support is controlled by (4), then the configuration is impossible.
...
in $U_C$. If $e=\delta$, $D_2$ lies beyond the C exit on $r_2$; if
$e=\alpha$, $D_4$ lies beyond the C exit on $r_4$. Both alternatives are
impossible. $\square$

This terminal is independent of the criticality of $T_0$. It closes every
applicable two-gap Vd0/T3-like row with $N_+\in\{0,1\}$, including the
two-gap output of the corrected Vd1 replacement.
...
role is supercritical. Normalize that role to $T_0$, write the gap as in (6),
and put
...
3. CE2 ends with the two thresholds supplied by $P_2,P_4$;
4. CE1 ends with the reverse return supplied by $P_4,P_3,P_2$, which gives
   $A_0>1-Q$.
...
Suppose a special role at $V_0$ has supported interval $[c,u]\subset r_1$
measured from $V_1$ toward $O$, and boundary endpoint $a$ on $e_{5,0}$. Put
$\varepsilon=1-u$ and $M=M_c^{\rm sup}$. Assume
...
on $T_5$ to be at least $M$, while strict supercriticality gives $B_1<M$.
Adding the four center-free handoffs yields
...
and the Vd1 neighboring-midpoint adapter in
[`4140_new`](../../4XXX_CE1CE2/41XX_Nplus1/414X_CE2_exactly_one_Vd1_Vd2_new/4140_new_one_Vd_finite_enclosure_assembly.md)
prove the same hypotheses (11)--(12). No type-specific center-hiding
calculation remains afterward.

## 6. Terminal E: residual radial separation

### Lemma 6.1

Parametrize $r_i$ by $xV_i$, $0\le x\le1$. Suppose the closed C role has
radial trace contained in $0\le x\le d_C$, and suppose there is a number
$c_{\rm loc}$ such that
...
Choose $x$ with $d_C<x<1-c_{\rm loc}$. The point $xV_i$ is missed by the C
role and the three local V roles. The remaining three V roles are excluded by
diameter locality. Hence $xV_i$ is uncovered. $\square$

In the adjacent one-Vd placement, the residual estimate and the one-third
radial envelope verify (13)--(14) on $r_2$. In the nonadjacent placement, the
point
...
lies beyond every relevant C trace and every local V trace. The two adapters
are proved in the established `4144` and `4146` sources and assembled in
`4140_new`.
...
the three asymmetric frontier witnesses replace the missing gap segment. The
two mixed support-cap overlaps are verified by the exact colocated
...
The corrected Vd1 two-chart replacement preserves the covered skeleton and
produces six nonsupercritical Vd0 roles. It is not a seventh terminal. Its
output gap rank must be recomputed.

| Output gap rank | Destination |
|---:|---|
| $0$ | the boundary-complete length row Z0 in [`2531`](../25XX_length_bounds/2531_length_budget_corollaries.md) |
| $1$ | Terminal A |
| $2$ | Terminal B |
...
| CE2, $N_+=1$, exactly one Vd1/Vd2 role | E, D, Strategy 1, or the replacement router, according to the exhaustive placement audit |

The first five rows are Theorems 2.1--7.1. The final row is the exhaustive
assembly in `4140_new`, whose endings are exactly those displayed. $\square$
...
| `4102`, `4103` | CE1 reverse return and CE2 thresholds | C |
| `4130_new` | T3-like endpoint inequalities | D or B |
| `4144`, `4146` | adjacent/nonadjacent Vd residual estimates | E |
| `4143` | Vd1 endpoint inequalities | D |
| `4147` | two-chart replacement | router |
