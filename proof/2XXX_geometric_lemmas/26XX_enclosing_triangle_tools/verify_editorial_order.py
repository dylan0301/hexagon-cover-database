...
check('fixed-start quadratics before Newton quotients',E.index('f_b(k)')<E.index('\\widehat k_b='))
check('rescaling aliases after fixed-start quotients',E.index('\\widehat k_b=')<E.index('\\widehat\\xi_-='))
...
F=expand(PAPER/'A_zero_gap_exact_certificate.tex')
check('Chebyshev definition before error budget',F.index('T_0(z)=1')<F.index('\\label{eq:f-chebyshev-budget}'))
check('explicit computation scope','mathematical supplement' in F and 'computer-assisted' in F)
check('no active old transcript','global tensor Bernstein expansions' not in F)
