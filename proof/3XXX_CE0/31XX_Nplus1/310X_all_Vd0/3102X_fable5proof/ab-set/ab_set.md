# The ab-set: an exact, case-by-case description
...
algebraic curves in counterclockwise order. Proofs: `ab_set_proofs.md`. Machine verification of
every case: `verify_ab_set.py`, summarized in `ab_set_verification.md`.
...
* $\eta_-:\ \mathcal E(a,b)>0$ — in the regime $\varepsilon\wedge(w_->0)$, a segment of $L_B$ is
  ...
  which is $-2^{-18}\operatorname{Res}_t\!\bigl(\mathcal D_B(t),\mathcal C_B(t)\bigr)$
...
`verify_ab_set.py` certifies for representative samples inside every case region (and near every
