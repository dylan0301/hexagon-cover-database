...
This file proves the description of $K(a,b)$ stated in `ab_set.md`. Notation is as there:
...
$\Phi$ stated in the bands table of `ab_set.md`, with endpoints:
...
`rho_of` in `verify_ab_set.py` implements).
...
covered by finitely many explicit algebraic curves, namely the catalog of `ab_set.md` §1 (the
...
**Lemma 5.1.** Let $\Gamma\subset C$ be the closed chain claimed for a case in `ab_set.md` §4
...
* **$c_A$-arcs** ($x$ with $|x-A|=1$): take $T$ with vertices $x$, $A$, and
  ...
  re-verified pointwise along every claimed arc in `verify_ab_set.py`).
...
symbolically/numerically in `verify_ab_set.py` over each case region (see
`ab_set_verification.md`).
...
(resultants; done in `derive_quartics.py`) yields the implicit quartics $E_{AO},E_{BO},E_{BA},E_{AB}$ printed
in `ab_set.md` §1. For $Q=Q_2$, (6.1) is the circle $|z-Q|=1$; this is how $c_A$ ($Q=A$) and $c_B$
...
s_A^{\mathrm{IV}},s_B^{\mathrm{IV}}$ of `ab_set.md` §1 after substituting
...
(`derive_thresholds.py`; also re-checked numerically). Redundant spurious factors like $(\sigma\pm1)$
...
4. Restrictions to the flush lines factor as
   ...
   with the cubics of `ab_set.md` §2 (parametrizations: on $L_A$,
...
6. $\eta_\pm$ (band I, $\varepsilon$-regime): the segment of $L_B$ between $E_{AB}$ and $E_{BO}$
   ...
   root: $\operatorname{Res}_t(\mathcal D_B,\mathcal C_B)=0$, the polynomial $\mathcal E(a,b)$
...
7. Outer-dip flush ($\lambda_B$): exposed iff the level $\frac{\sqrt3}2(1-b)$ exceeds the height of
   ...
   polynomial threshold $\mathcal G(a,b)$ of `ab_set.md` §3 (factor of degree 5; the discarded
...
For each case of `ab_set.md` §4, each listed arc is realizable on exactly the listed parameter
...
check of finitely many algebraic inequalities, done in `verify_ab_set.py`). Lemma 5.1 then gives
$S\subseteq K$.
...
1. **Curve-vs-curve on a sector.** E.g. in case I.6, for $\theta$ between $\theta(P_1)$ and
   ...
   **Lemma 9.1 (sector certification).** Let $f,g:(\theta;a,b)\mapsto\mathbb R$ be the radial
   ...
   of $\Sigma$, then $\operatorname{sign}(f-g)$ is constant on the interior of $\Sigma$, so a
   ...
   margins $\gg10^{-6}$, far above the $10^{-10}$ computational accuracy) in `verify_ab_set.py`.
...
`verify_ab_set.py`; every comparison is confirmed on every case region (dense grids, precision
...
The pruning facts, each verified on a fine grid in `verify_ab_set.py` ("region audit") and provable
...
(All checked exactly in sympy; script `derive_thresholds.py`, `derive_quartics.py`.)
