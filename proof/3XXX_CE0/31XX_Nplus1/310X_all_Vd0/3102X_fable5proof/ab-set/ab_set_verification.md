...
This file summarizes the machine verification of the case tables in `ab_set.md`
(script: `verify_ab_set.py`; supporting exact-algebra scripts developed during the derivation:
...
`ab_set_proofs.md` §1). For each direction $\theta$ the boundary radius
...
3. **Corner identities** — $P_1\in c_A\cap L_A\cap E_{AO}$, $P_2\in L_A\cap E_{BA}\cap E_{BO}$,
   ...
   *exactly* in sympy during the derivation (see `ab_set_proofs.md`, Appendix).
...
At least one interior sample of **every** case region of `ab_set.md` was verified — the numerically
...
The exact run log is produced by `python3 verify_ab_set.py` (region audit + corner identities +
the per-case table above). The full run passes end to end: **45/45 samples**, 0 failures,
concluding `== done: 45/45 samples passed ==`.
