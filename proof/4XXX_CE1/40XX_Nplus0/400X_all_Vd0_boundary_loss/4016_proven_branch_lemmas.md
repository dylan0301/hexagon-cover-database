...
The interval verifier in `proof/4XXX_CE1/40XX_Nplus0/400X_all_Vd0_boundary_loss/409X_experiments/4094_verify_full_L_interval.py` checks that every box satisfies either $\inf\Delta>0$ or $\sup H<0$.  The recorded run has $2307$ certified boxes and no unresolved boxes.
...
For $1<r<10$, the corrected interval verifier `proof/4XXX_CE1/40XX_Nplus0/400X_all_Vd0_boundary_loss/409X_experiments/4091_attempt_interval_L_Tplus_loss_exact_corrected.py` uses the exact loss equation
