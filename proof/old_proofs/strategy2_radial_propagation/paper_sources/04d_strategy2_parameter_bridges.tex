...
This source-only bridge derives the real parameters used in the finite
Strategy~2 inequalities.  It is the only file in this specification layer
where geometric data and the self-contained optimization variables coexist.
Every proposition below has one task: produce a point of an explicitly stated
real domain and identify the objective with the scalar quantity used in the
body.  No optimization inequality is proved here.  None of these bridge
propositions is encoded in the Lean scalar-statement project; Lean begins only after
the scalar input has been supplied.
...
\[
 ...
 T(c,x)=\Phi_c(x).
\]
For the endpoint parameters below,
\[
 \mathrm{Cap}(\mathbf e)
 =M_{c_-}(a_-)+M_{c_+}(a_+).
\]
...
The normalized one-gap endpoint tuple is
\[
...
Put
\[
 X=r-d,\qquad h_0=\frac{k}{2r},\qquad
...
It implies the body target \(Z>1-h_0\).
\end{proposition}
...
Normalizing the unique center midpoint to \(M_0\), and then imposing
\(\Haus(T_0\cap r_1)>0\), uses only the \(D_6\)-symmetry of the regular
...
\]
The boundary endpoints and the $T_0\cap r_1$ interval are
\[
...
u_\star=1+\beta-\tau.
\]
Let $A_1,A_5,C_1,C_5$ denote the actual endpoint reaches.  Choose the
cell-prescribed lower bounds
\[
 a_1\le A_1,\qquad a_5\le A_5,\qquad
 c_1\le C_1,\qquad c_5\le C_5.
\]
Every support-isolated endpoint parameter tuple has $c_5>1/2$.  If
...
\begin{proof}
The variables \(a_1,a_5\) are the selected far-side uncovered-suffix
boundary lower bounds, while \(c_1,c_5\) are the selected complementary
radial lower bounds.  On the high-radial branch,
apply \(F(c,x)=M_c(x)\) to both endpoint terms.
\end{proof}

\subsection{Adjacent exceptional-triangle domains}
...
\(\Omega_{\rm SC}^{V}\).  In particular, the condition
\(\Haus(T_0\cap r_5)=0\) is exactly
\[
...
\begin{proof}
The two raw endpoints on $r_5$ are
\[
...
\(\mu_5\le\lambda_5\).  The remaining equations are the $r_1$ graph and
the inequality expressing $M_1\in T_0$.
\end{proof}
...
\[
 \mathsf E(L,U;x),\qquad \mathcal R_{[L,U]}(x).
\]
...
Let \(p_1,q_1\) be the exact boundary reaches of a Vd triangle $T_i$, choose
the orientation with $\Haus(T_i\cap r_{i+1})>0$, and let \(t>0\) be its
corner parameter.  The far endpoint of $T_i\cap r_{i+1}$ is
\[
