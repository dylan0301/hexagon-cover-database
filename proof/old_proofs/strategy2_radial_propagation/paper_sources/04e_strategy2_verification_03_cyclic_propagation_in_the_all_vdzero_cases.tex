...
\[
 ...
 X=R-D,\qquad h_0=\frac{\eta+A+D}{2R}.
\]
The CE1 sign gives
...
\begin{equation}
 \left(\Phi_{1-D}\circ \Phi_{1-m_3}\circ \Phi_{1-A}\right)(h_0)>1-X.
 \label{eq:ce1-suffix}
\end{equation}
...
upper linear ranges would require $h_0\le A$ and $h_0\ge1-A$, respectively,
so both are impossible. On the constant or $Q_-$ branch range,
\[
...
\[
 ...
 =\frac{1-E}{2}>0.
\]
Thus $f_R(A_*)>0$.  Concavity again gives $f_R(A)>0$ on $(0,A_*)$, a
contradiction.  Both ranges are impossible, proving $X>1/2$.

Put $p_1=\Phi_{1-A}(h_0)$. If V triangle $3$ is not on the $Q_+$ branch, then
$p_1\le A+e(A)<3A+5A^2<29/64<1/2$, and
$2R(h_0-m_3)=\eta+D-A>\eta-P>0$. Extensivity gives
$p_1\ge h_0>m_3$, while $p_1<1/2<1-m_3$; thus both linear ranges are impossible.
On the constant, $Q_-$, or the low-$c$ tie one has $\overline M_{1-m_3}(p_1)\le p_1$, so
\[
 \Phi_{1-m_3}(p_1)\ge1-p_1>1-X.
\]
Extensivity of the final reverse map $\Phi_{1-D}$ preserves this strict bound
...
\begin{align*}
 p_1&>L_1:=(2-4A)h_0-(1-4A)A,\\
 p_2:=\Phi_{1-m_3}(p_1)&>L_2:=(2-5m_3)L_1-(1-5m_3)m_3.
\end{align*}
...
Since
\[
 L_2-2D-3D^2-\chi(D)=3D\left(\frac1{10}-D\right)>0,
\]
it is enough to prove $\chi(D)>0$. Exact differentiation gives
\[
 \chi'(D)=\frac{S(R,A)}{10R^2},\qquad
...
\]
Thus $\chi'(D)<0$ and $\chi(D)\ge\chi(D_h)$.

At $D=D_h$ one has $h_0=2A+5A^2$. Write $L_{2,h}=L_2(D_h)$.
Expansion gives
\[
...
assumed cover, every boundary gap in the V triangle traces must lie in a
